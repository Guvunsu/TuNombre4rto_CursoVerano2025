...
namespace OpenCocoWorld.Hoshi {
    ...
    public class Editor_HoshiLevelMaker : EditorWindow {
        ...
        [MenuItem("Open Coco World/Tools/Level Maker Hoshi")]

        public static void EditorDisplay() {
            Editor_HoshiLevelMaker window = GetWindow<Editor_HoshiLevelMaker>();
            window.titleContent = new GUIContent("Editor_HoshiLevelMaker");
        }
        ...
        void CreateGUI() {
            ...
            this._deleteAllGameEntititesButton = rootVisualElement.Q<Button>("DeleteAll");
            this._deleteAllGameEntititesButton.clicked += DeleteAllGameEntities;

            RetreiveFieldReferences();
        }
        ...
        protected void RetreiveFieldReferences() {
            this._operatingToggle = rootVisualElement.Q<Toggle>("Operating"); //with the same name as the viewport
            this._sceneTextField = rootVisualElement.Q<TextField>("Scene");
            this._prefabObjectField = rootVisualElement.Q<ObjectField>("Prefab");
            this._rootObjectField = rootVisualElement.Q<ObjectField>("Root");
        }
        protected void DeleteAllGameEntities() {
            Debug.Log("Editor_HoshiLevelMaker - DeleteAllGameEntities() " +
        ...
        protected virtual void DuringSceneGui(SceneView value) {
            if (_operatingToggle == null || _sceneTextField == null || _prefabObjectField == null || _rootObjectField == null || _deleteAllGameEntititesButton == null) {
                RetreiveFieldReferences();
            }
            ...
            if (!activeScene) {
                Debug.LogError("Editor_HoshiLevelMaker - DuringSceneGui() - " +
        ...
        public void CreateGameEntity(Vector2 mousePosition) {
            Debug.Log("Editor_HoshiLevelMaker - CreateGameEntity() - " +
            ...
            if (Physics.Raycast(_ray, out _raycastHit, 1000f)) {
                if (_raycastHit.collider.gameObject.layer == 15) //Layout layer
                {
                    ...
                    Debug.LogWarning("Editor_HoshiLevelMaker - CreateGameEntity() - " +
                ...
                Debug.LogError("Editor_HoshiLevelMaker - CreateGameEntity() - " +
